...
#                       .filter(or_(Teacher.start_work <= datetime(year=2021, month=1, day=1),
#                                   Teacher.start_work >= datetime(year=2022, month=12, day=31))).all()
#         # попереднiй виключає перiод
#         # наступний включає перiод
#         # .filter(or_(Teacher.start_work >= datetime(year=2021, month=1, day=1),
#         #             Teacher.start_work <= datetime(year=2022, month=12, day=31))).all()
#     for t in teachers:
...
if __name__ == '__main__':
    ...
    " далi - пiсля мiграцii таблицi [contacts] "
    # get_students_with_contacts()
    # get_info()
    " апдейт студента, котрий пiшов до вчителiв з [id] = [1, 2, 3] "
